        ...
        function()
          ...
          vim.schedule(function()
            ...
            if input_buf then
              ...
              vim.keymap.set("n", "<C-d>", require("fff.picker_ui").scroll_preview_down, { buffer = input_buf })
        ...
        bcommits = {
          actions = {
            ["enter"] = function(selected, opts)
              -- Extract the commit SHA (first field before space)
              -- Extract the commit SHA (first field)
              local commit = selected[1]:match("[^ ]+")

              -- Open vscode-diff comparing commit with current buffer
              vim.cmd(string.format("CodeDiff file %s", commit))
            end,
            -- Keep other default actions if needed
            ["ctrl-s"] = function(selected, opts)
              require("fzf-lua").actions.git_buf_split(selected, opts)
            end,
            ["ctrl-v"] = function(selected, opts)
              require("fzf-lua").actions.git_buf_vsplit(selected, opts)
            end,
          },
        },
      },
